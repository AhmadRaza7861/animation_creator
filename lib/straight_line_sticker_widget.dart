...
class _StraightLinePainter extends CustomPainter {
  ...
  bool? hitTest(Offset position) {
    ...
    final math.Point<double> A = math.Point(startPoint.dx, startPoint.dy);
    final math.Point<double> B = math.Point(endPoint.dx, endPoint.dy);
    final math.Point<double> P = math.Point(position.dx, position.dy);

    final double AB_x = B.x - A.x;
    final double AB_y = B.y - A.y;
    final double AP_x = P.x - A.x;
    final double AP_y = P.y - A.y;

    final double squaredLengthAB = AB_x * AB_x + AB_y * AB_y;
    if (squaredLengthAB == 0) {
      return AP_x * AP_x + AP_y * AP_y <= 25.0 * 25.0;
    }

    double t = (AP_x * AB_x + AP_y * AB_y) / squaredLengthAB;
    t = t.clamp(0.0, 1.0);

    final double projection_x = A.x + AB_x * t;
    final double projection_y = A.y + AB_y * t;
    
    final double distSq = math.pow(P.x - projection_x, 2).toDouble() + math.pow(P.y - projection_y, 2).toDouble();
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 3. Highlight/Border for visibility (optional, keeping it simple)
